...
public class ApodController : ControllerBase
{
    ...
    /// <param name="count">Optional count for random gallery (max 10)</param>
    [HttpGet]
    public async Task<ActionResult<ApodDto>> GetApod([FromQuery] string? date = null, [FromQuery] int? count = null)
    {
        // Cache key based on date or "today"
        ...
        try
        {
            var apod = await _nasaApiService.GetApodAsync(date, count);
